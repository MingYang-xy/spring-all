...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	// 此上下文当前是否处于活动状态的标志，AtomicBoolean线程安全的boolean对象
	...
	public AbstractApplicationContext() {
		// 获取资源匹配处理器，加载资源用的，资源：xml文件.yam文件等配置
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			// 这里的this.applicationStartup默认就是DefaultApplicationStartup
			// 当前可以使用setXX方法自定义Startup，自定义Startup后这里就是一个可扩展的点
			// 默认情况下DefaultXX中，start(String name) name根本没有使用，就是起一个标记作用
			StartupStep contextRefresh = this.applicationStartup.start("spring.context.refresh");

			// Prepare this context for refreshing.
			// 准备刷新上下文：设置当前容器运行状态为active，初始化默认的environment、监听器
			prepareRefresh();

			// Tell the subclass to refresh the internal bean factory.
			// 通过调用子类实现的refreshBeanFactory 获取一个全新的beanFactory
			ConfigurableListableBeanFactory beanFactory = obtainFreshBeanFactory();

			// Prepare the bean factory for use in this context.
			// 初始化beanFactory，设置一些属性值
			prepareBeanFactory(beanFactory);

			try {
				// Allows post-processing of the bean factory in context subclasses.
				// 执行beanFactoryPostProcessor接口中的postProcessBeanFactory方法，
				// 当前抽象类并没有具体实现该方法，提供给后续使用者扩展
				...
				// 初始化messageSource，即国际化：就是配置好不同国家的语言
				initMessageSource();
				...
				// 刷新前扩展，留给子类实现
				// springboot项目的ServletWebServerApplicationContext在这一步去new了Tomcat
				onRefresh();
				...
				// 重置spring的核心类的一些缓存，因为可能不再需要元数据来处理单例bean
				resetCommonCaches();
				contextRefresh.end();
			}
		}
	...
	protected void prepareRefresh() {
		...
		// 初始化当前环境的配置信息，当前类无实现，留给子类扩展
		initPropertySources();

		// Validate that all properties marked as required are resolvable:
		// see ConfigurablePropertyResolver#setRequiredProperties
		// getEnvironment()：新建了默认的一些系统环境变量信息，
		// validateRequiredProperties()：校验一些必需的配置key，如果一个配置key不存在，直接报错。
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// 添加BeanFactory的后置处理器，比如xxxAwareProcessor
		beanFactory.addBeanPostProcessor(new ApplicationContextAwareProcessor(this));
		// 忽略这些接口的实现，暂时不去处理Aware接口
	...
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 实例化并执行所有BeanFactoryPostProcessor
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// 如果没有BeanFactoryPostProcessor，则注册一个默认的配置来源占位符解析器
		...
		// 这里执行了this.frozenBeanDefinitionNames = StringUtils.toStringArray(this.beanDefinitionNames);
		// 将beanName缓存起来了
		beanFactory.freezeConfiguration();

		// Instantiate all remaining (non-lazy-init) singletons.
		// 实例化剩余的非懒加载的单利bean，在此之前，已经实例化过beanFactory的PostProcessor和一些框架自身的bean
