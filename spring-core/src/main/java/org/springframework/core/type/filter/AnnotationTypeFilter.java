...
public class AnnotationTypeFilter extends AbstractTypeHierarchyTraversingFilter {
	 ...
	 * @param considerMetaAnnotations 是否匹配元注解
	 * @param considerInterfaces 是否匹配接口
	 */
	public AnnotationTypeFilter(
			Class<? extends Annotation> annotationType, boolean considerMetaAnnotations, boolean considerInterfaces) {
		// 这里new AnnotationTypeFilter(Component.class)时，默认匹配元注解，不匹配接口
	...
	protected boolean matchSelf(MetadataReader metadataReader) {
		...
		// 这里this.annotationType就是@Component
		// metadata.hasAnnotation(this.annotationType.getName())表示是否直接被@Component标记
		// metadata.hasMetaAnnotation(this.annotationType.getName())表示当前类的注解的父级注解是否被@Component标记
		// @Repository、@Service、@Controller都是@Component的派生注解，所以这里只检查了是否被@Component标记
